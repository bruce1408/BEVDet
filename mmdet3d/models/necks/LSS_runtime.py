...
class QuickCumsum(torch.autograd.Function):
    ...
    def forward(ctx, x, geom_feats, ranks):
        x = x.cumsum(0)
        kept = torch.ones(x.shape[0], device="cpu", dtype=torch.bool)
...
class ViewTransformerLiftSplatShoot(BaseModule):
    ...
    def voxel_pooling(self, geom_feats, x):
        ...
        # print('geom_feats shape is: ', geom_feats.shape) ([8, 6, 59, 19, 53, 3])
...
if __name__ == "__main__":
    ...
    # x = FakeData(20, (1, 6, 3, 256, 704), 4, transforms.ToTensor())
    # x: [4, 6, 3, 256, 704]
    # rots: [4,6,3,3]
    # trans: [4,6,3]
    # intrins: [4,6,3,3]
    # post_rots: [4,6,3,3]
    # post_trans: [4,6,3]
    x = torch.randn(1, 6, 512, 16, 44)
    rots = torch.randn(1, 6, 3, 3)
    trans = torch.randn(1, 6, 3)
    intrins = torch.randn(1, 6, 3, 3)
    post_rots = torch.randn(1, 6, 3, 3)
    post_trans = torch.randn(1, 6, 3)
    net = ViewTransformerLiftSplatShoot()
    net.to("cpu")
    input = [x, rots, trans, intrins, post_rots, post_trans]
